...
describe("Utils", () => {
  ...
  describe(hslToRgb.name, () => {
    ...
    it.each(testData)("converts HSL to RGB correctly", (h, s, l, r, g, b) => {
      const [er, eg, eb] = hslToRgb(h, s, l);
      expectClose(er, r);
      expectClose(eg, g);
      expectClose(eb, b);
    });
  ...
  describe(rgbToHsv.name, () => {
    const testData: [number, number, number, number, number, number][] = [
      ...
      [0, 0, 255, 2 / 3, 1, 1],
      [255, 0, 170, 8 / 9, 1, 1],
      [128, 128, 128, 0, 0, 0.5019607843137255],
      [255, 255, 255, 0, 0, 1],
    ];

    it.each(testData)(
      "converts RGB to HSV correctly",
      (r, g, b, h, s, v) => {
        const [hslH, hslS, hslV] = rgbToHsv(r, g, b);
        expectClose(hslH, h);
        expectClose(hslS, s);
        expectClose(hslV, v);
      },
  ...
  describe(hsvToRgb.name, () => {
    const testData: [number, number, number, number, number, number][] = [
      ...
      [2 / 3, 1, 1, 0, 0, 255],
      [8 / 9, 1, 1, 255, 0, 170],
      [0, 0, 0.5019607843137255, 128, 128, 128],
      [0, 0, 1, 255, 255, 255],
    ];

    it.each(testData)("converts HSV to RGB correctly", (h, s, v, r, g, b) => {
