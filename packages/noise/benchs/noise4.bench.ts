...
bench("Perlin 3D", () => {
  let sum = 0;
  for (let i = 0; i < dataSize; i++) {
    const n = perlin.noise4(dx[i], dy[i], dz[i], dw[i]);
...
bench("Simplex 3D", () => {
  let sum = 0;
  for (let i = 0; i < dataSize; i++) {
    const n = simplex.noise4(dx[i], dy[i], dz[i], dw[i]);
    if (!Number.isFinite(n)) {
      const params = {
        x: dx[i],
        y: dy[i],
...
bench("Value 3D", () => {
  let sum = 0;
  for (let i = 0; i < dataSize; i++) {
    sum += value.noise4(dx[i], dy[i], dz[i], dw[i]);
  }
  sink += sum;
});
...
bench("White 3D", () => {
  let sum = 0;
  for (let i = 0; i < dataSize; i++) {
    sum += white.noise4(dx[i], dy[i], dz[i], dw[i]);
  }
  sink += sum;
});
