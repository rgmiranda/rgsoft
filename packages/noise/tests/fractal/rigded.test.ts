...
import { Simplex, Ridged } from "../../src";

const G2 = (3 - Math.sqrt(3)) / 6;

describe(Ridged.name, () => {
  const noise = new Ridged(new Simplex("test-seed"), 4, 2, 0.5);
  ...
  describe("noise1", () => {
    ...
    it("should generate one values on integers", () => {
      const value1 = noise.noise1(0);
      const value2 = noise.noise1(1);
      const value3 = noise.noise1(10);
      expect(value1).toBeCloseTo(1);
      expect(value2).toBeCloseTo(1);
      expect(value3).toBeCloseTo(1);
    });
  });
